...
import { AMEVATensorDisposedError } from "../src/errors";
...
describe("TensorRegistry", () => {
  ...
  it("fails on missing handle", () => {
    expect(() => _globalRegistry.get("invalid_handle")).toThrow(AMEVATensorDisposedError);
  });
  ...
  it("fails on disposed handle access", () => {
    ...
    expect(() => _globalRegistry.get(handle)).toThrow(AMEVATensorDisposedError);
  });
