...
var device$1 = /*#__PURE__*/Object.freeze({
    ...
    setDeviceLostCallback: setDeviceLostCallback
});
...
function assertNoDynamicShaderSource(_input) {
    throw new AMEVATensorSecurityError("User input cannot be used directly in WGSL shader generation.");
}
/**
...
function validateShape(shape, dtype, expectedByteLength) {
    ...
    if (shape.length < 1 || shape.length > 4) {
        throw new AMEVATensorShapeError(`Shape rank must be between 1 and 4, got ${shape.length}.`);
    }
    let elements = 1;
...
class TensorRegistry {
    ...
    dispose(handle) {
        ...
        Promise.resolve().then(function () { return device$1; }).then(({ getDevice }) => {
            try {
...
export { AMEVATensorDTypeError, AMEVATensorDeviceError, AMEVATensorDisposedError, AMEVATensorError, AMEVATensorQuotaExceededError, AMEVATensorSecurityError, AMEVATensorShapeError, AMEVATensorWebGPUUnavailableError, QuotaManager, add, assertAllowedKernelName, assertAllowedShaderConstant, assertNoDynamicShaderSource, assertSafeShaderIdentifier, assertStaticShaderSourceOnly, assertWasmRange, cloneToFloat32Array, dispose, ensureFloat32Array, getAdapter, getDevice, getQueue, getTensorInfo, init, initWebGPU, isAvailable, mapBufferAsync, matmul, mul, random, read, readMappedInto, registerPyodideBridge, relu, relu_backward, resetRuntimeMemory, setDeviceLostCallback, transpose, uploadFloat32Array, validateDType, validateShape };
