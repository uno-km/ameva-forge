...
describe("validateWasmRange", () => {
  ...
  it("fails on negative offset", () => {
    expect(() => assertWasmRange(-10, 100, wasmLength)).toThrow(AMEVAForgeSecurityError);
  });
  ...
  it("fails on unsafe integer", () => {
    expect(() => assertWasmRange(10.5, 100, wasmLength)).toThrow(AMEVAForgeSecurityError);
  });

  it("fails on out-of-bounds", () => {
    expect(() => assertWasmRange(900, 200, wasmLength)).toThrow(AMEVAForgeSecurityError);
  });

  it("boundary test for byteLength > wasmByteLength - offset", () => {
    expect(() => assertWasmRange(800, 200, wasmLength)).not.toThrow();
    expect(() => assertWasmRange(800, 201, wasmLength)).toThrow(AMEVAForgeSecurityError);
  });
});
