...
describe("QuotaManager", () => {
  ...
  it("fails on hard limit exceeded", () => {
    const q = new QuotaManager(1000, 500);
    expect(() => q.reserve(1001)).toThrow(AMEVAForgeQuotaExceededError);
  });

  it("fails on invalid byteLength", () => {
    const q = new QuotaManager(1000, 500);
    expect(() => q.reserve(-100)).toThrow(AMEVAForgeQuotaExceededError);
    expect(() => q.reserve(10.5)).toThrow(AMEVAForgeQuotaExceededError);
  });

  it("fails on release over allocated", () => {
    const q = new QuotaManager(1000, 500);
    q.reserve(300);
    expect(() => q.release(400)).toThrow(AMEVAForgeQuotaExceededError);
  });
