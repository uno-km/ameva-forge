...
`;
    ...
    })(exports.GGMLType || (exports.GGMLType = {}));
    class GGUFStreamer {
...
`;
     ...
     */
    exports.VAEDecoderErrorCode = void 0;
    ...
    })(exports.VAEDecoderErrorCode || (exports.VAEDecoderErrorCode = {}));
    ...
    function assertPositiveSafeInteger(name, value) {
        if (!Number.isSafeInteger(value) || value <= 0) {
            throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_INVALID_DIMENSION, `${name} must be a positive safe integer, received: ${value}`);
        }
    }
    function checkedElementCount(name, dimensions, maxLimit) {
        ...
        for (const d of dimensions) {
            ...
            if (total > Math.floor(maxLimit / d)) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_RESOURCE_LIMIT_EXCEEDED, `${name} element count exceeds maximum limit of ${maxLimit}`);
            }
            total *= d;
        }
        return total;
    ...
    function assertLength$1(name, actual, expected) {
        if (actual !== expected) {
            throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHT_SHAPE_MISMATCH, `${name} length mismatch: expected ${expected}, received ${actual}`);
        }
    }
    function assertAllFinite(name, values, isWeight = false) {
        for (let i = 0; i < values.length; i++) {
            if (!Number.isFinite(values[i])) {
                const code = isWeight
                    ? exports.VAEDecoderErrorCode.VAE_NON_FINITE_WEIGHT
                    : exports.VAEDecoderErrorCode.VAE_NON_FINITE_INPUT;
    ...
    class VAEDecoder {
        ...
        static unscaleLatents(latents, scaleFactor = VAEDecoder.DEFAULT_SCALE_FACTOR) {
            if (!Number.isFinite(scaleFactor) || scaleFactor <= 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_SCALE_FACTOR_INVALID, `scaleFactor must be a finite positive number, received: ${scaleFactor}`);
            }
        ...
        static decode(latents, latentWidth, latentHeight, weights, scaleFactor = VAEDecoder.DEFAULT_SCALE_FACTOR, limits = DEFAULT_LIMITS) {
            // 1. 가중치 객체 필수 검증
            if (!weights) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHTS_REQUIRED, 'VAE decoder weights are required. Refusing to decode with synthetic weights.');
            }
            // 2. 정확한 3-stage 검증 (4개 이상 침묵 허용 차단)
            if (!weights.upBlocks || weights.upBlocks.length !== VAE_DECODER_ARCHITECTURE.upBlockCount) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_UPBLOCK_COUNT_MISMATCH, `VAE decoder requires exactly ${VAE_DECODER_ARCHITECTURE.upBlockCount} upsample stages in upBlocks, received: ${weights.upBlocks?.length ?? 0}`);
            }
            ...
            for (let stage = 0; stage < 3; stage++) {
                ...
                if (!stageBlock?.upsampleConvWeight) {
                    throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHTS_REQUIRED, `Missing VAE upsample convolution weight at stage ${stage}`);
                }
                if (!stageBlock?.normGamma || !stageBlock?.normBeta) {
                    throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHTS_REQUIRED, `Missing VAE upsample norm parameters at stage ${stage}`);
                }
        ...
        static decodeLatentToRGB(latents, latentWidth, latentHeight, outWidth, outHeight, weights, scaleFactor, limits) {
            ...
            if (outWidth !== expectedW || outHeight !== expectedH) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_OUTPUT_SCALE_MISMATCH, `decodeLatentToRGB scale mismatch: requested ${outWidth}x${outHeight}, but latent ${latentWidth}x${latentHeight} scales to ${expectedW}x${expectedH}`);
            }
        ...
        static conv2d(x, inC, outC, H, W, weight, bias, kernelSize = 3, padding = 1) {
            ...
            if (!Number.isInteger(kernelSize) || kernelSize <= 0 || kernelSize % 2 === 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_CONV_CONTRACT_INVALID, `conv2d requires a positive odd kernel size, received: ${kernelSize}`);
            }
            const expectedPadding = Math.floor(kernelSize / 2);
            if (padding !== expectedPadding) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_CONV_CONTRACT_INVALID, `conv2d only supports same-padding convolution: kernelSize=${kernelSize}, required padding=${expectedPadding}, received padding=${padding}`);
            }
        ...
        static groupNorm(x, C, H, W, G, gamma, beta, eps = 1e-5) {
            ...
            if (!Number.isInteger(G) || G <= 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_GROUP_COUNT_INVALID, `GroupNorm group count must be positive safe integer: ${G}`);
            }
            if (C % G !== 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_GROUP_DIVISIBILITY_ERROR, `GroupNorm requires C divisible by G: C=${C}, G=${G}`);
            }
            if (!Number.isFinite(eps) || eps <= 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_EPS_INVALID, `GroupNorm eps must be a finite positive number, received: ${eps}`);
            }
            if (gamma.length !== C || beta.length !== C) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHT_SHAPE_MISMATCH, `GroupNorm affine parameter mismatch: C=${C}, gamma=${gamma.length}, beta=${beta.length}`);
            }
    ...
    function assertPositiveSafeInt(name, val) {
        if (!Number.isSafeInteger(val) || val <= 0) {
            throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_INVALID_DIMENSION, `${name} must be a positive safe integer, received: ${val}`);
        }
    }
    ...
    class AutoencoderKLDecoder {
        ...
        static decode(latents, latentWidth, latentHeight, weights, scaleFactor = AutoencoderKLDecoder.DEFAULT_SCALE_FACTOR) {
            if (!weights) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHTS_REQUIRED, 'AutoencoderKL weights are strictly required. Refusing to decode with synthetic weights.');
            }
            if (!Number.isFinite(scaleFactor) || scaleFactor <= 0) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_SCALE_FACTOR_INVALID, `scaleFactor must be a finite positive number, received: ${scaleFactor}`);
            }
            ...
            if (!weights.upBlocks || weights.upBlocks.length !== 4) {
                throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_UPBLOCK_COUNT_MISMATCH, `AutoencoderKL requires exactly 4 up_block stages, received: ${weights.upBlocks?.length ?? 0}`);
            }
            ...
            for (let stage = 0; stage < 4; stage++) {
                ...
                if (!upStage.resnets || upStage.resnets.length !== 3) {
                    throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHT_SHAPE_MISMATCH, `AutoencoderKL stage ${stage} requires exactly 3 ResNet blocks, received: ${upStage.resnets?.length ?? 0}`);
                }
                ...
                if (upStage.hasUpsample) {
                    ...
                    if (!upStage.upsampleConvWeight) {
                        throw new VAEDecoderError(exports.VAEDecoderErrorCode.VAE_WEIGHTS_REQUIRED, `Missing upsampleConvWeight at AutoencoderKL stage ${stage}`);
                    }
    ...
    class WebGPUDiffusionPipeline {
        ...
        async generate(options) {
            ...
            if (!options.unetWeights) {
                throw new DiffusionPipelineError(exports.DiffusionPipelineErrorCode.UNET_FORWARD_NOT_IMPLEMENTED, 'UNet weights are strictly required. Refusing to simulate denoising with heuristic decay formulas. Real UNet execution graph required.');
            }
     ...
     */
    exports.VisionErrorCode = void 0;
    ...
    class ClassicalCV {
        ...
        static toGrayscale(rgba, width, height) {
            if (rgba.length !== width * height * 4) {
                throw new VisionError(exports.VisionErrorCode.BUFFER_SIZE_MISMATCH, `RGBA buffer length mismatch: expected ${width * height * 4}, received ${rgba.length}`);
            }
        ...
        static canny(grayInput, width, height, lowThreshold = 0.1, highThreshold = 0.3) {
            if (lowThreshold >= highThreshold || lowThreshold < 0) {
                throw new VisionError(exports.VisionErrorCode.THRESHOLD_INVALID, `lowThreshold (${lowThreshold}) must be strictly less than highThreshold (${highThreshold}) and >= 0`);
            }
    ...
    class CLIPVisionEncoder {
        ...
        bias) {
            ...
            if (width % p !== 0 || height % p !== 0) {
                throw new VisionError(exports.VisionErrorCode.INVALID_IMAGE_DIMENSIONS, `Image dimensions must be divisible by patch size (${p}), received: ${width}x${height}`);
            }
    ...
    class TTSEngine {
        ...
        static synthesize(text, sampleRate = TTSEngine.DEFAULT_SAMPLE_RATE, f0 = 140.0) {
            ...
            if (sampleRate < 8000 || sampleRate > 48000) {
                throw new TTSError(exports.TTSErrorCode.TTS_INVALID_SAMPLE_RATE, `Sample rate must be between 8000 and 48000, received: ${sampleRate}`);
            }
     ...
     */
    exports.TokenizerErrorCode = void 0;
    ...
    })(exports.TokenizerErrorCode || (exports.TokenizerErrorCode = {}));
    ...
    class BPETokenizer {
        ...
        initFromConfig(config) {
            if (!config.vocab || config.vocab.length === 0) {
                throw new TokenizerError(exports.TokenizerErrorCode.TOKENIZER_EMPTY_VOCAB, 'Cannot initialize BPETokenizer with empty vocabulary array.');
            }
        ...
        static fromGGUFMetadata(metadata) {
            ...
            if (!tokens || tokens.length === 0) {
                throw new TokenizerError(exports.TokenizerErrorCode.TOKENIZER_EMPTY_VOCAB, 'GGUF metadata does not contain valid "tokenizer.ggml.tokens".');
            }
        ...
        encode(text, addBos = true) {
            if (!this.isInitialized) {
                throw new TokenizerError(exports.TokenizerErrorCode.TOKENIZER_NOT_INITIALIZED, 'BPETokenizer must be initialized with a vocabulary before encoding.');
            }
        ...
        decode(tokenIds, skipSpecialTokens = true) {
            if (!this.isInitialized) {
                throw new TokenizerError(exports.TokenizerErrorCode.TOKENIZER_NOT_INITIALIZED, 'BPETokenizer must be initialized with a vocabulary before decoding.');
            }
            let pieces = [];
    ...
    exports.ALL_MODAL_CAPABILITIES = ALL_MODAL_CAPABILITIES;
    ...
    exports.AUTOENCODER_KL_CAPABILITY = AUTOENCODER_KL_CAPABILITY;
    exports.AllModalOrchestrator = AllModalOrchestrator;
    exports.AutoencoderKLDecoder = AutoencoderKLDecoder;
    exports.BPETokenizer = BPETokenizer;
    exports.CLIPTextEncoder = CLIPTextEncoder;
    exports.CLIPTokenizer = CLIPTokenizer;
    exports.CLIPVisionEncoder = CLIPVisionEncoder;
    exports.ClassicalCV = ClassicalCV;
    ...
    exports.GGUFTensorMapper = GGUFTensorMapper;
    exports.GROUP_NORM_APPLY_WGSL = GROUP_NORM_APPLY_WGSL;
    exports.GROUP_NORM_STATS_WGSL = GROUP_NORM_STATS_WGSL;
    exports.InferenceWorkerHandler = InferenceWorkerHandler;
    exports.KERNEL_REGISTRY = KERNEL_REGISTRY;
    exports.LLMEngine = LLMEngine;
    exports.LLMError = LLMError;
    exports.LLMTextGenerator = LLMTextGenerator;
    exports.LoadedModelSession = LoadedModelSession;
    exports.ModelLoader = ModelLoader;
    exports.OFFICIAL_MODEL_PRESETS = OFFICIAL_MODEL_PRESETS;
    exports.QuotaManager = QuotaManager;
    exports.ResNetBlock = ResNetBlock;
    exports.SILU_BACKWARD_WGSL = SILU_BACKWARD_WGSL;
    exports.SILU_WGSL = SILU_WGSL;
    exports.STTEngine = STTEngine;
    exports.STTError = STTError;
    exports.STT_MEL_WGSL = STT_MEL_WGSL;
    exports.STT_STFT_WGSL = STT_STFT_WGSL;
    ...
    exports.WorkerSession = WorkerSession;
    ...
    exports.embedding_backward = embedding_backward;
